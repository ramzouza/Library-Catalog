...
class LoanService {
    ...
    static allLoanUser(userId) {
        return connection.query("Select * from resource as r LEFT JOIN resource_line_item as rll on r.id = rll.resource_id where user_id ='" + userId + "' ")
    }

    static allLoanAdmin() {
        return connection.query("Select * from resource as r LEFT JOIN resource_line_item as rll on r.id = rll.resource_id where date_due != '' and date_due != 'Never'")
    }
